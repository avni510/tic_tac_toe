...
module TicTacToe
  class HardComputer < Player
    ...
    def score(board)
      game_eval = GameEval.new
      if game_eval.won?(board.cells)
        if game_eval.winning_player_marker(board.cells, @marker, @opponent_marker) == @marker
          return 10
        else
          return -10
        end
      else game_eval.tied(board.cells, @marker, @opponent_marker)
        return 0
      end        
    end
