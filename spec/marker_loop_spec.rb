module TicTacToe
  ...
  describe MarkerLoop do 
    ...
    it "returns the markers for both players" do 
      allow(mock_console_ui).to receive(:select_marker).and_return("X")
    ...
    it "prompts the user to select a marker" do 
      ...
      expect(mock_console_ui).to receive(:select_marker).and_return("X")
    ...
    context "Player 1 chooses 'O'" do 
      it "returns 'X' for Player 2" do 
        expect(mock_console_ui).to receive(:select_marker).and_return("O")

        allow(mock_console_ui).to receive(:puts_markers).with("O", "X")
      ...
      it "displays the markers of each player" do 
        allow(mock_console_ui).to receive(:select_marker).and_return("O")
    ...
    context "the player enters an invalid marker" do 
      it "prompts to the user to enter another marker" do 
        expect(mock_console_ui).to receive(:select_marker).and_return('p', 'X')
